...
def create_metric_card(
    title, value, delta=None, suffix="", description=None, help=None
):
    display_value = value
        ...
        with st.spinner("Loading survey data..."):
            ...
            for response in responses:
                ...
                if last_response_time:
                    response_times.append(
                        (timestamp - last_response_time).total_seconds() / 3600
                    )
            ...
            for _, response in df.iterrows():
                if response["type"] == "open_ended":
                    ...
                    question_engagement[response["question"]] = question_engagement.get(
                        response["question"], []
                    )
                    question_engagement[response["question"]].append(
                        min(words / 10, 1)
                    )  # Cap at 1, 10 words = full score
                elif response["type"] == "likert_scale":
                    # Score based on non-neutral answers (1,2 or 4,5 vs 3)
                    score = (
                        abs(float(response["answer"]) - 3) / 2
            ...
            avg_engagement = (
                np.mean([np.mean(scores) for scores in question_engagement.values()])
                * 100
            )
            ...
            tabs = st.tabs(
                ["📊 Response Analysis", "📝 Text Analysis", "🎯 Insights Dashboard"]
            )
            ...
            with tabs[0]:
                ...
                mc_questions = len(
                    df[df["type"] == "multiple_choice"]["question"].unique()
                )
                rating_questions = len(
                    df[df["type"] == "likert_scale"]["question"].unique()
                )
                ...
                stats_cols[3].metric(
                    "Average Rating",
                    f"{df[df['type'] == 'likert_scale']['answer'].astype(float).mean():.1f}/5",
                )
                        ...
                        if not multiple_choice.empty:
                            for question in multiple_choice["question"].unique():
                                ...
                                with st.expander(f"📊 {question}", expanded=True):
                                    ...
                                    value_counts = (
                                        data["answer"].value_counts().reset_index()
                                    )
                        ...
                        if not likert.empty:
                            for question in likert["question"].unique():
                                with st.expander(f"📈 {question}", expanded=True):
                                    ...
                                    stats_cols[1].metric(
                                        "Median Rating", f"{median_rating:.1f}"
                                    )
                                    stats_cols[2].metric(
                                        "Most Common", f"{mode_rating:.1f}"
                                    )
            ...
            with tabs[1]:
                ...
                if not open_ended.empty:
                    ...
                    avg_subjectivity = sentiments.apply(
                    ...
                    st.markdown(
                        f"""
                        **Interpretation**: The average sentiment score of **{avg_polarity:.2f}** reflects the overall tone of responses:
                        - **Above 0.3** generally indicates positive sentiment.
                        - **Between -0.3 and 0.3** suggests neutral or balanced feedback.
                        - **Below -0.3** signals a negative sentiment. 

                        This score helps identify the general mood of participants towards the topic, which is useful for tailoring improvements.
                        """
                    )

                    # Sentiment Metrics
                    sentiment_counts = sentiments.apply(
                        lambda x: (
                            "Positive"
                            if x.polarity > 0.3
                            else "Negative" if x.polarity < -0.3 else "Neutral"
                        )
                    ).value_counts()

                    total_responses = len(sentiments)
                    positive_pct = (
                        sentiment_counts.get("Positive", 0) / total_responses
                    ) * 100
                    negative_pct = (
                        sentiment_counts.get("Negative", 0) / total_responses
                    ) * 100
                    neutral_pct = (
                        sentiment_counts.get("Neutral", 0) / total_responses
                    ) * 100
                    ...
                    st.markdown(
                        """
            **Significance**: Understanding the breakdown of sentiment can be crucial for identifying specific areas of improvement or positive feedback. 
            If a high percentage of responses are positive, this could indicate that current strategies are resonating well. Conversely, higher negative feedback might warrant immediate action.
            """
                    )
                    ...
                    keywords = open_ended["answer"].apply(
                        lambda x: [
                    ...
                    keyword_freq = (
                        pd.Series(all_keywords).value_counts().head(10).reset_index()
                    )
                    keyword_freq.columns = [
                        "Keyword",
                        "Count",
                    ...
                    fig = px.bar(
                        keyword_freq, x="Keyword", y="Count", title="Top Keywords"
                    )
                    fig.update_layout(
                        xaxis_title="Keyword",
                        yaxis_title="Count",
                    ...
                    st.markdown(
                        """
            **Interpretation**: Identifying the most common keywords offers insight into the main topics of interest or concern among respondents. This information can guide you in focusing on the most impactful areas for improvement.
            """
                    )
                    ...
                    st.markdown(
                        """
            **Significance**: The word cloud visually represents the most frequently mentioned words, providing a quick overview of prevalent themes in the responses.
            """
                    )
                    ...
                    # Topic Modeling (LDA)
                    st.markdown("### 🗂️ Topic Modeling: Discover Key Themes in Responses")
                    ...
                    # Retrieve topics with associated keywords and probabilities
                    topics = lda_model.show_topics(formatted=False, num_words=5)

                    # Display topics as interactive cards with interpretations
                    st.markdown("#### Key Topics Identified")

                    for i, topic in enumerate(topics):
                        keywords_probabilities = ", ".join([f"{word} ({probability:.2f})" for word, probability in topic[1]])

                        st.markdown(
                            f"""
                            <div style='background-color: rgba(0,0,0,0.05); padding: 15px; border-radius: 10px; margin-bottom: 15px;'>
                                <h4>🟢 Topic {i + 1}</h4>
                                <p><strong>Top Keywords:</strong> {keywords_probabilities}</p>
                                <p style='font-size: 14px; color: rgba(255,255,255,0.7);'>
                                    <em>Interpretation:</em> This topic is likely related to <strong>themes around {', '.join([word for word, _ in topic[1][:2]])}</strong>.
                                    The top keywords give insights into common concerns, interests, or feedback areas.
                                </p>
                            </div>
                            """,
                            unsafe_allow_html=True
                    ...
                    st.markdown(
                        """
                        **Interpretation**: Topic modeling uses Latent Dirichlet Allocation (LDA) to uncover main themes in the responses. 
                        Each topic is represented by key terms with associated probabilities, helping reveal underlying patterns.
                        """
                    )
                    ...
                    """.format(
                            (
                                "positive"
                                if avg_polarity > 0.3
                                else "negative" if avg_polarity < -0.3 else "neutral"
                            ),
                            (
                                "highly"
                                if avg_subjectivity < 0.3
                                else "moderately" if avg_subjectivity < 0.7 else "less"
                            ),
                            round((1 - avg_subjectivity) * 100, 1),
                            "positive" if positive_pct > negative_pct else "negative",
                            sentiment_time.groupby(sentiment_time["timestamp"].dt.date)[
                                "polarity"
                            ]
                            .mean()
                            .idxmax()
                            ...
                            (
                                "improving"
                                if sentiment_time["polarity"].corr(
                                    pd.Series(range(len(sentiment_time)))
                                )
                                > 0.1
                                else (
                                    "declining"
            ...
            with tabs[2]:
                ...
                with col1:
                    ...
                    day_engagement = (
                        df.groupby(["day_of_week", "trainee_email"])
                        .agg(
                            {
                                "timestamp": "size",
                                "answer": lambda x: (
                                    len(str(x).split()) if isinstance(x, str) else 1
                                ),
                            }
                        )
                        .reset_index()
                    ...
                    day_stats = (
                        day_engagement.groupby("day_of_week")
                        .agg({"timestamp": "count", "answer": "mean"})
                        .reset_index()
                    )
                ...
                with col2:
                    ...
                    quality_metrics = (
                        ...
                        .agg(
                            {
                                "answer": lambda x: np.mean(
                                    [
                                        len(str(i).split()) if isinstance(i, str) else 0
                                        for i in x
                                    ]
                                ),
                    ...
                    fig = px.scatter(
                        ...
                        labels={
                            "answer": "Avg Response Length",
                            "trainee_email": "Unique Respondents",
                        },
                    )
                ...
                with insight_cols[0]:
                    ...
                    """.format(
                            day_stats.loc[
                                day_stats["timestamp"].idxmax(), "day_of_week"
                            ],
                            hour_counts.idxmax(),
                            df["answer"]
                            .apply(
                                lambda x: (
                                    len(str(x).split()) if isinstance(x, str) else 0
                                )
                            )
                            .mean(),
                        ),
                ...
                with insight_cols[2]:
                    ...
                    """.format(
                            ...
                            (
                                ...
                                and response_times[-1] > response_times[0]
                                else "Stable"
                            ),
                        ),
                        unsafe_allow_html=True,
                ...
                """.format(
                        ...
                        (
                            ...
                            if df[df["type"] == "open_ended"]["answer"]
                            .apply(len)
                            .mean()
                            > 50
